...
if ffmpeg_path:
    ...
    st.error("FFmpeg introuvable. Ajoutez 'ffmpeg' dans packages.txt ou installez-le.")
    st.stop()
...
try:
    ...
    st.error(f"moviepy manquant : {e}")
    st.stop()

# ====================================================================
# Configuration de la page
# ====================================================================
st.set_page_config(page_title="Générateur de Pub IA", page_icon="📹", layout="wide")

# ---- CSS en une seule ligne (SANS guillemets triples) ----
st.markdown("<style>.main{background:#f8f9fa}.stButton>button{background:#ff4b4b;color:white;border-radius:30px;padding:0.5rem 2rem;font-weight:bold;border:none;transition:0.3s}.stButton>button:hover{background:#e60000;transform:scale(1.02)}.block-container{padding-top:2rem;padding-bottom:2rem}.title{font-size:2.5rem;font-weight:700;color:#1e2a3a;text-align:center;margin-bottom:1rem}.subtitle{text-align:center;color:#555;margin-bottom:2rem}.contact-info{background:#1e2a3a;padding:1rem;border-radius:12px;color:white;text-align:center;margin-top:1rem}</style>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    voice_lang = st.selectbox("Langue de la voix", ["en-US", "en-GB", "fr-FR", "es-ES", "de-DE", "it-IT", "pt-BR"])
    ...
    bg_music_file = st.file_uploader("Musique (MP3/WAV optionnel)", type=["mp3", "wav"])
    product_images = st.file_uploader("Images produit (max 10)", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
    if len(product_images) > 10:
        product_images = product_images[:10]

    st.markdown("---")
    st.markdown('<div class="contact-info">📞 (509) 4738-5663<br>📧 deslandes78@gmail.com</div>', unsafe_allow_html=True)

# ---- Entrées principales ----
col1, col2 = st.columns([2, 1])
with col1:
    product_name = st.text_input("Nom du produit / service", placeholder="ex. Prisme Transfer Haïti")
    product_description = st.text_area("Description / caractéristiques", placeholder="Décrivez votre produit...", height=150)
    call_to_action = st.text_input("Appel à l'action", placeholder="Inscrivez-vous, Achetez maintenant...")
...
def generate_video(product_name, description, cta, bg_color, images, voice, music_file, logo_data):
    ...
    closing_message = ("Pour plus d'informations, contactez-nous au (509) 4738-5663 ou par courriel à deslandes78@gmail.com. "
    ...
    st.info(f"Durée voix : {duration:.1f}s")
    
    slides = []
    if images and len(images) > 0:
        slide_count = min(len(images), int(duration/1.5))
...
# ---- Génération ----
if generate_btn:
    if not product_name or not product_description or not call_to_action:
        ...
        with st.spinner("🎬 Génération en cours..."):
            try:
                ...
                video_path = generate_video(product_name, product_description, call_to_action,
                                            bg_color, image_list, selected_voice, bg_music_file, logo_data)
                ...
                if os.path.exists(video_path) and os.path.getsize(video_path) > 0:
                    with video_placeholder.container():
                        ...
                        with open(video_path, "rb") as f:
                            video_bytes = f.read()
                        st.info(f"Taille : {len(video_bytes)//1024} Ko")
                        b64 = base64.b64encode(video_bytes).decode()
                        st.markdown(f'<video width="100%" controls autoplay><source src="data:video/mp4;base64,{b64}" type="video/mp4"></video>', unsafe_allow_html=True)
                        st.download_button("⬇️ Télécharger", data=video_bytes,
